        ...
        for forecast_horizon in forecast_horizons:
            ...
            for rope in rope_scaling:
                for context_length in context_length_list:
                    ...
                    train_data_lds = to_deepar_format(train_data, freq = freq)
                    ...
                    # save all metrics
                    agg_metrics_df.to_csv(f'outputs/tuning/all_metrics_{data}_{train}_{forecast_horizon}_{context_length}_{rope_scaling}.csv', index=False)
...
best_hyperparameters = all_metrics_df.groupby(['dataset', 'train_size', 'forecast_horizon']).apply(lambda x: x.nsmallest(1, 'RMSE')).reset_index(drop=True)[['dataset','train_size', 'forecast_horizon', 'context_length', 'rope_scaling']]

# add row to best_hyperparameters
# add row to best_hyperparameters
best_hyperparameters.loc[len(best_hyperparameters)] = ['climate', 'small', 10, 100, True]
best_hyperparameters.loc[len(best_hyperparameters)] = ['climate', 'small', 50, 100, True]
best_hyperparameters.loc[len(best_hyperparameters)] = ['weather', 'small', 10, 100, True]
best_hyperparameters.loc[len(best_hyperparameters)] = ['weather', 'small', 50, 100, True]
