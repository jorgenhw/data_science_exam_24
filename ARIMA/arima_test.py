...
for train in train_size:
    for forecast_horizon in forecast_horizons:
        ...
        os.makedirs(f'ARIMA/outputs/test/Partition_horizon_{forecast_horizon}_train_{train}', exist_ok=True)

        # save forecast_df
        forecast_df.to_csv(f'ARIMA/outputs/test/Partition_horizon_{forecast_horizon}_train_{train}/forecast_df.csv', index=False)
...
all_metrics_df.to_csv('ARIMA/outputs/test/arima_hyperparameters.csv', index=False)

# summarise bases on train and horizon both mean and std
all_metrics_df.groupby(['train_size', 'forecast_horizon']).agg({'rmse': ['mean', 'std'], 'mape': ['mean', 'std']}).to_csv('ARIMA/outputs/test/arima_summary.csv')