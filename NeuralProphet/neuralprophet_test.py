...
for data in dataset:
    for train in train_size:
        for forecast_horizon in forecast_horizons:
            ...
            n_changepoints=int(df[(df['train_size'] == train) & (df['forecast_horizon'] == forecast_horizon)]['n_changepoints'].values[0])
            n_lags=int(df[(df['train_size'] == train) & (df['forecast_horizon'] == forecast_horizon)]['n_lags'].values[0])
            ...
            os.makedirs(f'NeuralProphet/outputs/test/Partition_horizon_{forecast_horizon}_train_{train}_{data}', exist_ok=True)

            # save forecast_df
            forecast_df.to_csv(f'NeuralProphet/outputs/test/Partition_horizon_{forecast_horizon}_train_{train}_{data}/forecast_df.csv', index=False)
...
all_metrics_df.groupby(['dataset', 'train_size', 'forecast_horizon']).agg({'mae': ['mean', 'std'], 'mse': ['mean', 'std'], 'rmse': ['mean', 'std'], 'mape': ['mean', 'std']}).to_csv('NeuralProphet/outputs/test/np_summary.csv')